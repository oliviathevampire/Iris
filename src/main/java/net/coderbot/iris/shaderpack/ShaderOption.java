...
import net.coderbot.iris.Utils;
...
public abstract class ShaderOption {
	...
	public String getDescriptionText() {
		String desc = Utils.normalize(this.description);
		desc = Utils.removePrefix(desc, "//");
		return I18n.translate("option." + this.getName() + ".comment", desc);
	}
	...
	public void addPaths(String[] newPaths) {
		...
		for (String newPath : newPaths) {
			if (!pathList.contains(newPath)) {
				this.paths = (String[]) Utils.addObjectToArray(this.paths, newPath);
			}
		}
	...
	public boolean isChanged() {
		return !Utils.equals(this.value, this.valueDefault);
	}
	...
	public String getNameText() {
		return I18n.translate("option." + this.name, this.name);
	}

	public String getValueText(String val) {
		return I18n.translate("value." + this.name + "." + val, val);
	}
	...
	public float getIndexNormalized() {
		if (this.values.length <= 1) {
			...
			int index = getIndex(this.value, this.values);
			return index < 0 ? 0.0F : index / (this.values.length - 1.0F);
		}
	}

	public void setIndexNormalized(float f) {
		if (this.values.length > 1) {
			f = Utils.limit(f, 0.0F, 1.0F);
	...
	public String toString() {
		return "" + this.name + ", value: " + this.value + ", valueDefault: " + this.valueDefault + ", paths: " + Utils.arrayToString(this.paths);
	}
}
