...
public record ShaderLine(net.coderbot.iris.shaderpack.ShaderLine.Type type, String name,
	...
	public Vec3i getValueIVec3() {
		if (this.value == null) {
			...
			str = Utils.removePrefix(str, "ivec3");
			str = Utils.trim(str, " ()");
			String[] parts = Utils.tokenize(str, ", ");
			if (parts.length != 3) {
				return null;
			} else {
				int[] vals = new int[3];
				...
				for (int i = 0; i < parts.length; ++i) {
					String part = parts[i];
					int val = Utils.parseInt(part, Integer.MIN_VALUE);
	...
	public Vec2f getValueVec2() {
		if (this.value == null) {
			return null;
		} else {
			String str = this.value.trim();
			str = Utils.removePrefix(str, "vec2");
			str = Utils.trim(str, " ()");
			String[] parts = Utils.tokenize(str, ", ");
			if (parts.length != 2) {
				return null;
				...
				for (int i = 0; i < parts.length; ++i) {
					String part = parts[i];
					part = Utils.removeSuffix(part, new String[]{"F", "f"});
					float f = Utils.parseFloat(part, Float.MAX_VALUE);
	...
	public Vector4f getValueVec4() {
		if (this.value == null) {
			...
			String str = this.value.trim();
			str = Utils.removePrefix(str, "vec4");
			str = Utils.trim(str, " ()");
			String[] parts = Utils.tokenize(str, ", ");
			if (parts.length != 4) {
				return null;
