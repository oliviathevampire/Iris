...
public class Utils {
				...
				if (!startsWithMaskSingle(str, s1, wildCharSingle)) {
					...
					if (!endsWithMaskSingle(str, s2, wildCharSingle)) {
						...
						for (String value : list) {
							if (value.length() > 0) {
								int k = indexOfMaskSingle(str, value, i, wildCharSingle);
								if (k < 0) {
									return false;
								}
								i = k + value.length();
	...
	public static String listToString(List<String> list, String separator) {
		if (list == null) {
			...
			StringBuilder stringbuffer = new StringBuilder(list.size() * 5);
	...
	public static Object[] addObjectToArray(Object[] arr, String obj) {
		if (arr == null) {
	...
	public static Object[] collectionToArray(Collection<Object> coll, Class<?> elementClass) {
		if (coll == null) {
			return null;
